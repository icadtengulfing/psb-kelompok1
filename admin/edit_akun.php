...
$query_data = mysqli_query( $koneksi, "SELECT * FROM pendaftar WHERE id_pendaftar='$_GET[edit]'" );
$data = mysqli_fetch_array( $query_data );
$foto_profil_path = (!empty($data['foto_profil']) && $data['foto_profil'] != 'default-profile.jpg') 
                   ? "../dashboard/uploads/" . $data['foto_profil'] 
                   ...
                   if (isset($_POST['ubah_akun'])) {
                    ...
                    // default: tetap gunakan foto lama
                    $foto_profil_update = "";
                    ...
                    if (isset($_FILES['fotoProfil']) && $_FILES['fotoProfil']['error'] === 0) {
                        ...
                        if (in_array($_FILES['fotoProfil']['type'], $allowed_types)) {
                            if (move_uploaded_file($_FILES['fotoProfil']['tmp_name'], $target_file)) {
                                $foto_profil_update = $file_name; // simpan nama file ke db
                            }
                        }
                    } else {
                        // ambil foto lama dari database
                        $cek = mysqli_query($koneksi, "SELECT foto_profil FROM pendaftar WHERE id_pendaftar='$id_pendaftar'");
                        if ($row = mysqli_fetch_assoc($cek)) {
                            $foto_profil_update = $row['foto_profil'];
                        }
                    }
                    // update data
                    $query_edit = "UPDATE pendaftar 
                                   SET nama_lengkap_ortu = '$nama_lengkap_ortu',
                                       email = '$email',
                                       password = '$hash_password',
                                       no_hp = '$no_hp',
                                       foto_profil = '$foto_profil_update'
    ...
    <section class="p-4 lg:p-8 sm:ml-64">
        <div class="mt-26 lg:mt-24">
            ...
            <div class="flex flex-col gap-8 mb-8">
                ...
                <input type="hidden" name="id_pendaftar" value="<?php echo htmlspecialchars($data['id_pendaftar']); ?>">
                    <div class="mb-5">
                        <label for="fotoProfil" class="block mb-2 text-md md:text-lg font-medium text-[var(--txt-primary)]">
                            Foto Profil
                        </label>
                        <input type="file" id="fotoProfil" name="fotoProfil"
                            class="bg-transparent border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:bg-[var(--bg-primary2)]/10 focus:border-[var(--bg-primary2)] block w-full py-2.5 px-3.5 cursor-pointer">
                        <small class="block text-gray-500 mt-1">
                            Foto Profil saat ini:
                        </small>
                        <img src="<?php echo htmlspecialchars($foto_profil_path); ?>"
                          alt="user photo"
                          class="w-40 h-40 rounded-full me-3 shadow-md object-cover">
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" id="namaLengkap" name="namaLengkapEdit"
                            ...
                            placeholder="Masukkan Nama Lengkap" value="<?php echo $data['nama_lengkap_ortu']; ?>" />
                    </div>
                    
                    <div class="mb-5">
                        <label for="email" class="block mb-2 text-md md:text-lg font-medium text-[var(--txt-primary)]">
                            Email
                        </label>
                        <input type="email" id="email" name="emailEdit"
                            class="bg-transparent border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:bg-[var(--bg-primary2)]/10 focus:border-[var(--bg-primary2)] block w-full py-2.5 px-3.5"
                            placeholder="Masukkan Email" value="<?php echo $data['email']; ?>" />
                    </div>
                    
                    <div class="mb-5">
                        <label for="password" class="block mb-2 text-md md:text-lg font-medium text-[var(--txt-primary)]">
                            Password
                        </label>
                    ...
                    <div class="mb-5">
                        <label for="noHp" class="block mb-2 text-md md:text-lg font-medium text-[var(--txt-primary)]">
                            No. HP
                        </label>
                        <input type="number" id="noHp" name="no_hpEdit"
                            class="bg-transparent border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:bg-[var(--bg-primary2)]/10 focus:border-[var(--bg-primary2)] block w-full py-2.5 px-3.5"
                            placeholder="Masukkan No. Hp" value="<?php echo $data['no_hp']; ?>" />
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" id="NIK" name="NIK" aria-label="disabled input"
                            class="py-2.5 px-3.5 mb-5 bg-[var(--bg-primary2)]/20 hover:bg-[var(--bg-primary2)]/30 border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:border-[var(--bg-primary2)] block w-full cursor-not-allowed"
