...
$query_data = mysqli_query($koneksi, "SELECT * FROM jalur_pendaftaran WHERE id_jalur='$_GET[edit]'");
$data = mysqli_fetch_array($query_data);

if (isset($_POST['ubah_akun'])) {
  $id_jalur = mysqli_real_escape_string($koneksi, $_POST['id_jalur']);
  $jenis_jalur = mysqli_real_escape_string($koneksi, $_POST['jenis_jalur']);
  $kuota = mysqli_real_escape_string($koneksi, $_POST['kuota']);
  // hash password
      ...
      <div class="flex flex-col gap-8 mb-8">
        ...
        <div class="mb-5">
          ...
          <div class="mb-5">
            ...
            <input type="text" id="jenis_jalur" name="jenis_jalur"
              class="bg-transparent border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:bg-[var(--bg-primary2)]/10 focus:border-[var(--bg-primary2)] block w-full py-2.5 px-3.5"
              placeholder="Masukkan Jenis_Jalur" value="<?php echo $data['jenis_jalur']; ?>" />
          </div>

          <div class="mb-5">
            <label for="kuota" class="block mb-2 text-md md:text-lg font-medium text-[var(--txt-primary)]">
            Kuota
            </label>
            <input type="number" id="kuota" name="kuota"
              class="bg-transparent border border-[var(--txt-primary)]/50 text-[var(--txt-primary)] text-md rounded-lg focus:ring-[var(--bg-primary2)] focus:bg-[var(--bg-primary2)]/10 focus:border-[var(--bg-primary2)] block w-full py-2.5 px-3.5"
              placeholder="Masukkan Kuota Jalur" value="<?php echo $data['kuota']; ?>" />
          </div>
