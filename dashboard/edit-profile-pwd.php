...
if (isset($_POST['ubah_password'])) {
    ...
    // Ambil password lama dari database (yang sudah di-hash)
    $query_old_pass = "SELECT password FROM pendaftar WHERE nik = '$nik_escaped'";
    $result_old_pass = mysqli_query($koneksi, $query_old_pass);
    
    if ($result_old_pass && mysqli_num_rows($result_old_pass) > 0) {
        $hashed_password_db = mysqli_fetch_assoc($result_old_pass)['password'];
        
        // Verifikasi password lama
        if (password_verify($passwordLama, $hashed_password_db)) {
            // ✅ Password lama benar, redirect ke halaman input password baru
            $_SESSION['verify_password'] = true; // Set session untuk keamanan
            header('Location: edit-profile-pwd-1.php');
            exit;
        } else {
            ...
            <div class="grid grid-cols-1 xl:grid-cols-[3fr_1fr] gap-4 lg:gap-8 mb-8">
                <div
                    ...
                    <form action="" method="post" class="w-full">
                        ...
                        <div class="mb-6">
                            <label class="block mb-2 text-lg text-[var(--txt-primary)]" for="passwordLama">
                                Password Lama
                            </label>
                            <input class="text-lg w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-[var(--bg-primary2)] bg-transparent text-[var(--txt-primary)]" type="password" id="passwordLama" name="passwordLama" placeholder="Masukkan Password Lama" required />
                        </div>
