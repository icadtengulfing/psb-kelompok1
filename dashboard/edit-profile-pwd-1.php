...
if (isset($_POST['ubah_password'])) {
  $passwordBaru = mysqli_escape_string($koneksi, $_POST['passwordBaru']);
  $konfirmasiPassword = mysqli_escape_string($koneksi, $_POST['konfirmasiPassword']);
      // Cek apakah password baru dan konfirmasi password cocok
      if ($passwordBaru === $konfirmasiPassword) {
          // Hash password baru sebelum disimpan
          $hashed_password_new = password_hash($passwordBaru, PASSWORD_DEFAULT);

          // Update password di database
          $update_password_query = "UPDATE pendaftar SET password = ? WHERE nik = ?";
          $update_password_stmt = mysqli_prepare($koneksi, $update_password_query);
          mysqli_stmt_bind_param($update_password_stmt, "ss", $hashed_password_new, $nik);
...
<head>
    ...
    <title>Profile Murid Website SMK - PSB</title>
        ...
        <div class="h-full px-5 pb-4 overflow-y-auto bg-[var(--bg-primary3)]">
            ...
            <div
                ...
                <a href="edit-profile.php"
                    class="cursor-pointer flex items-center w-full text-sm rounded-xl hover:shadow-md group bg-[var(--bg-primary2)]/20 hover:bg-[var(--bg-primary2)]/10 px-4 py-3 transition duration-400"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[5fr_1fr] gap-4 lg:gap-8 mb-20 lg:mb-8">
                <div
                    ...
                    <p class="text-lg lg:text-2xl text-[var(--txt-primary)] mt-0 lg:mt-2">
                        Profil pengguna akun
                    </p>
                </div>
                <div class="w-full relative">
                    <a href="../index.php"
                        class="absolute top-0 right-0 text-[var(--txt-secondary)] bg-gradient-to-r from-red-500 via-red-600 to-red-700 hover:bg-gradient-to-br font-bold rounded-lg shadow-lg text-md lg:text-lg lg:rounded-xl cursor-pointer hover:shadow-xl px-6 py-2.5 text-center me-2 mb-2">Keluar
                    </a>
                </div>
            </div>
            <div class="grid grid-cols-1 mb-8">
                <div
                    class="flex flex-col lg:flex-row justify-start items-center p-4 lg:p-6 gap-2 lg:gap-10 rounded-xl bg-[var(--bg-primary3)] border border-[var(--bg-primary2)]/30 shadow-md">
                    <img src="<?php echo $foto_profil_path; ?>"
                        class="shadow-lg h-30 lg:h-50 rounded-full ms-0 lg:ms-6"
                        style="width: 200px; height: 200px; object-fit: cover;"
                        alt="Image Daftar Reguler Dashboard">
                        <form method="POST" enctype="multipart/form-data">
                        <button type="submit" class="tombol-custom text-center w-auto text-[var(--txt-secondary)] bg-[var(--bg-primary2)] hover:bg-[var(--bg-secondary)] transition duration-300 focus:ring-3 focus:ring-[var(--bg-secondary)] font-bold rounded-lg text-lg lg:text-xl px-5 py-2.5 mt-6 focus:outline-none cursor-pointer" onclick="document.getElementById('foto-profil').click()">
                        Pilih Foto
                        <input type="file" id="foto-profil" name="foto_profil" accept=".jpg,.jpeg,.png" required style="display: none;">
                        <button type="submit" class="text-center w-auto text-[var(--txt-secondary)] bg-[var(--bg-primary2)] hover:bg-[var(--bg-secondary)] transition duration-300 focus:ring-3 focus:ring-[var(--bg-secondary)] font-bold rounded-lg text-lg lg:text-xl px-5 py-2.5 mt-6 focus:outline-none cursor-pointer ms-6" name="upload_foto">Upload</button>
                    </form>
                </div>
            </div>
            <div class="grid grid-cols-1 xl:grid-cols-[3fr_1fr] gap-4 lg:gap-8 mb-8">
                <div
                    class="flex flex-col justify-center items-start p-6 lg:p-8 rounded-xl bg-[var(--bg-primary3)] border border-[var(--bg-primary2)]/30 shadow-md">
                    <form action="" method="post" class="w-full">
                        <h1 class="mb-6 text-xl md:text-4xl font-bold text-[var(--txt-primary)]">Ubah Kata Sandi</h1>
                        <div class="mb-6">
                            <label class="block mb-2 text-lg text-[var(--txt-primary)]" for="passwordBaru">
                                Password Baru
                            </label>
                            <input class="text-lg w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-[var(--bg-primary2)] bg-transparent text-[var(--txt-primary)]" type="password" id="passwordBaru" name="passwordBaru" placeholder="Masukkan Password Baru" required />
                        </div>
                        <div class="mb-6">
                            <label class="block mb-2 text-lg text-[var(--txt-primary)]" for="konfirmasiPassword">
                              Konfirmasi Password Baru
                            </label>
                            <input class="text-lg w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-[var(--bg-primary2)] bg-transparent text-[var(--txt-primary)]" type="password" id="konfirmasiPassword" name="konfirmasiPassword" placeholder="Masukkan Konfirmasi Password Baru" required />
                        </div>
                        <button type="submit" name="ubah_password"
                            class="cursor-pointer w-full lg:w-2/12 py-3 px-4 bg-[var(--bg-primary2)] hover:bg-[var(--bg-secondary)] text-[var(--txt-secondary)] font-bold rounded-xl shadow-md text-xl transition-all duration-500">Ubah</button>
                    </form>
                </div>
                <div
                    class="flex flex-col justify-start items-start p-6 lg:p-8 rounded-xl bg-[var(--bg-primary3)] border border-[var(--bg-primary2)]/30 shadow-md">
                    <h1 class="mb-8 text-xl md:text-4xl font-bold text-[var(--txt-primary)]">Edit Profile</h1>
                    <a href="edit-profile.php"
                        class="flex items-center gap-2 lg:gap-4 w-full mb-5 hover:bg-[var(--bg-primary2)]/20 rounded-2xl hover:shadow-lg transition duration-300 px-2 py-1">
                        <img src="../assets/img/edit-profile-user.png" alt="Image Edit Profile Pengguna">
                        <span class="text-lg lg:text-2xl font-medium text-[var(--txt-primary2)]">Profile Pengguna</span>
                    </a>
                    <a href="edit-profile-pwd.php"
                        class="
                        flex items-center gap-2 lg:gap-4 w-full mb-5 bg-[var(--bg-primary2)]/20 hover:bg-[var(--bg-primary2)]/10 rounded-2xl shadow-sm transition duration-300 p-3">
                        <img src="../assets/img/edit-profile-pwd.png" alt="Image Edit Password Pengguna">
                        <span class="text-lg lg:text-2xl font-medium text-[var(--txt-primary2)]">Kata Sandi</span>
                    </a>
                </div>
            </div>
        </div>
